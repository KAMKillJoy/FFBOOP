...
class Commander:
    ...
    def build_ffmpeg_command(self, file: str) -> str:
        ...
        if passes == "One-Pass":
            cmd = (
                f'ffmpeg -y -i "{file}" {vf} -c:v libvpx-vp9 -b:v 0 '
                f'-deadline {preset} -crf {crf} -row-mt 1 '
                f'-c:a libopus -b:a 128k "{output_file}_1pass.{container}"'
            )
        else:  # Two-Pass
            nol = "NUL" if os.name == "nt" else "/dev/null"
            cmd = (
                f'ffmpeg -y -i "{file}" {vf} -c:v libvpx-vp9 -b:v 0 '
                f'-deadline {preset} -crf {crf} -row-mt 1 -pass 1 -an -f null {nol} && '
                f'ffmpeg -y -i "{file}" {vf} -c:v libvpx-vp9 -b:v 0 '
                f'-deadline {preset} -crf {crf} -row-mt 1 -pass 2 '
                f'-c:a libopus -b:a 128k "{output_file}_2pass.{container}"'
            )
